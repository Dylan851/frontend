...
class GameState {
  ...
  // para que no reaparezcan una vez recogidos.
  ...
  int    currentXp    = 680;
  int    maxXp        = 1000;
  String currentMapId = 'jungle';  // Aldea Canta — mapa principal (Kanto Ruta 1)
  ...
  // ── Mapa ──────────────────────────────────
  // 0 = sin posición guardada → usar default del mapa
  double savedX = 0;
  double savedY = 0;
  // Id del mapa al que pertenece savedX/savedY — si cambia el mapa,
  // no reutilizamos la posición guardada (podría estar dentro de un árbol).
  String? savedMapId;

  // ── Personaje seleccionado ─────────────────
  // id que mapea a los sprites: assets/images/player/{selectedCharacter}_down.png …
  ...
  void completeMinigame(String animalId, int stars) {
    // Se considera "ganado" con >= 2 estrellas; ahí desbloqueamos el animal
    // en la colección (aparece con su imagen e info tipo Wikipedia).
    if (stars >= 2) {
  ...
  void collectMapItem(String itemId) {
    if (collectedMapItems.contains(itemId)) return; // ya recogido
