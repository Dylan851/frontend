...
class WoodPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final base =
        bg ?? (emerald ? const Color(0xFF143421) : AppColors.parchment);
    return Container(
      padding: padding,
...
class GlassBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: borderColor ?? AppColors.borderWhite, width: 1.5),
        ),
        child: child,
      );
}
...
class CurrencyChip extends StatelessWidget {
  ...
  final Color? accent;
  const CurrencyChip(
      {super.key, required this.icon, required this.value, this.accent});
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color: AppColors.amber.withOpacity(0.55), width: 1.5),
            boxShadow: [
              BoxShadow(
          ...
          child: const Icon(Icons.arrow_back_ios_new_rounded,
              color: AppColors.parchment, size: 15),
        ),
      );
}
...
class CornerRibbon extends StatelessWidget {
  ...
  final Color color;
  const CornerRibbon(
      {super.key, required this.label, this.color = AppColors.amber});
...
abstract class GameTone {
  ...
  static const Color panelDark =
      Color(0xFF1B2812); // dark panel fill (greenish)
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      painter: _PixelFramePainter(
          radius: radius, innerFill: innerFill, studs: goldStuds),
      child: Padding(
...
class _PixelFramePainter extends CustomPainter {
  ...
  final bool studs;
  _PixelFramePainter(
      {required this.radius, required this.innerFill, required this.studs});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final hi = RRect.fromRectAndRadius(
        const Offset(3, 3) & Size(size.width - 6, 2), Radius.circular(1));
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(const Offset(7, 7) & Size(size.width - 14, 1.5),
...
class GameLogo extends StatelessWidget {
  ...
  const GameLogo(
      {super.key, required this.title, this.subtitle, this.fontSize = 44});
  ...
  Widget build(BuildContext context) {
    final outline = <Shadow>[
      ...
      const Shadow(
          color: Color(0xFF1A0E04), offset: Offset(2, 0), blurRadius: 0),
      const Shadow(
          color: Color(0xFF1A0E04), offset: Offset(0, -2), blurRadius: 0),
      const Shadow(
          color: Color(0xFF1A0E04), offset: Offset(0, 2), blurRadius: 0),
      const Shadow(
          color: Color(0xFF1A0E04), offset: Offset(2, 2), blurRadius: 0),
      const Shadow(
          color: Color(0xFF1A0E04), offset: Offset(-2, 2), blurRadius: 0),
      const Shadow(
          color: Color(0x88000000), offset: Offset(0, 6), blurRadius: 8),
    ];
        ...
        builder: (_, __) => Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color:
                    const Color(0xFF6BE095).withOpacity(0.3 + 0.3 * _c.value),
                blurRadius: 22,
                spreadRadius: 1,
              ),
            ],
...
class _DapplePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // tiny fireflies
    final fp = Paint()
      ..color = const Color(0xFFFFE48A).withOpacity(0.9)
      ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 1.5);
    for (int i = 0; i < 12; i++) {
      canvas.drawCircle(
        Offset(rng.nextDouble() * size.width,
            size.height * (0.4 + rng.nextDouble() * 0.55)),
...
class SunRaysPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant SunRaysPainter old) =>
